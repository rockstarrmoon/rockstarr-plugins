...
description: "This skill should be used when the user asks to \"draft a newsletter\", \"write this week's newsletter\", \"draft the email newsletter\", or names a topic (or set of topics) they want turned into a newsletter. It produces a single-shot email newsletter draft in 03_drafts/content/ with subject line options, a preheader, a personal intro, one to three short sections, and a clear CTA. Newsletters are shorter and more conversational than blog posts and ship without an outline-first gate."
---
...
- User picks a newsletter-format topic from
  `02_inputs/content-topics_*.md`.
- User hands over a free-text topic and says "newsletter".

Newsletters are single-shot in v0.1. No outline step. If the user
wants a longer, more structured piece, steer them to `draft-article`
or `outline-blog + draft-blog`.
...
- Either a topic (title + angle) from the user, OR a pointer to a
  line item in `02_inputs/content-topics_*.md`.
- `/rockstarr-ai/00_intake/stack.md` — so the CTA points to a real
  destination (the client's booking link, form, or next-email
  trigger).
...
3. Stack — to confirm where the CTA leads (calendar link, offer
   page, reply-to address).
4. First-party KB files (`kb_scope: owned`) relevant to the topic.
5. Third-party KB files (`kb_scope: third_party`) — allowed as
   reference / link-worthy external material, never as voice.
6. Recent newsletter entries in `05_published/_publish.log` — to
...
produced_by: "rockstarr-content-bot/draft-newsletter@0.1.0"
...
## CTA
[The CTA line — a single specific next action. Format it the way
it will appear in the email (button-style line, link, or reply
prompt).]

Destination: [URL or action reference]
...
2. **One idea per issue.** If the topic has multiple beats, the
   ...
   is too big for a newsletter and belongs in a blog or article.
...
- Do not draft a newsletter longer than ~1000 words in v0.1. If
  the idea demands it, write a blog or article instead.
