---
name: outline-blog
description: "This skill should be used when the user asks to \"outline a blog post\", \"plan the blog\", \"structure the post before drafting\", or names a topic they want to blog about. It is the first step of the two-step blog flow and must complete before draft-blog runs. It produces an outline file in 03_drafts/content/ with working title, angle, audience, H2 structure, CTA, target keyword, rough word-count, and cited first-party KB sources. The outline must be explicitly approved by the user before any drafting begins."
---

# outline-blog

The outline gate for blog posts. Blog drafts are expensive to fix
after the fact — a bad angle, a missing H2, or a CTA that does not
fit the offer can force a full rewrite. This skill catches those
problems cheaply, before any prose is written.
...
- User picks a blog topic from a `content-topics_*.md` file.
- User asks to outline a post on a fresh topic.
- A previous draft was rejected for structural reasons and the
  reviewer wants to re-outline before the next attempt.
...
3. The source topic — either the user-supplied topic or the line
   item from `02_inputs/content-topics_*.md`. Preserve the angle
...
produced_by: "rockstarr-content-bot/outline-blog@0.1.0"
...
(Blog posts usually land in 3 to 5 H2s. More than 6 and you are
writing an article — flag it and ask the user whether to switch to
`draft-article`.)
