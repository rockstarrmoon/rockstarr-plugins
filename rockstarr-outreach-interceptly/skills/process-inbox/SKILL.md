---
name: process-inbox
description: "This skill should be used in the daily outreach loop after preview-queue-interceptly, or when the user says \"process the inbox\", \"walk unread replies\", or \"handle today's Interceptly inbox\". Filters Interceptly Inbox then Replied, walks unread oldest-first, runs qualify-lead, hands off to rockstarr-reply. Two modes: foreground calls present-for-approval inline and executes channel-side work; background stages drafts and returns staged_paths so daily-loop can fire notify-reply-ready once. Unreads come before My Tasks. Refuses if confirm-session has not passed."
---
