...
- `/rockstarr-ai/00_intake/icp-qualifications.md` exists (produced
  by `rockstarr-reply:capture-icp-qualifications`). The baseline
  qualification rules are the default for this campaign.
- `/rockstarr-ai/00_intake/stack.md` has `outreach_tool =
  interceptly`. If the tool is anything else, refuse and point at
  the right variant.
- `/rockstarr-ai/00_intake/interceptly-accounts.md` exists with at
  least one managed account row. If missing, point at
...
4. `00_intake/style-guide.md` — read in full. Pay attention to:
   ...
   - Channel Adaptation → LinkedIn replies — warm-ICP pattern
     subsection (written by
     `rockstarr-reply:capture-warm-reply-pattern`; the shape is
     the same pattern LinkedIn replies use, so the sequence's
     post-accept follow-ups inherit it)
...
`icp-qualifications.md` (owned by `rockstarr-reply`).
...
---
slug: <slug>
derived_from: icp-qualifications.md (baseline, owned by rockstarr-reply)
...
edit via `rockstarr-reply:capture-icp-qualifications` and re-run
this skill.)
...
2. **Follow-up #1** (after accept; default day+3). One-line value
   ...
   pattern (the subsection of `style-guide.md` written by
   `rockstarr-reply:capture-warm-reply-pattern`) — same shape as
   a reply, since the lead just accepted and a reply is the
   natural next move.
...
- Exit conditions: any reply (handed to `rockstarr-reply`'s
  per-reply pipeline), accept without reply (enters follow-up
  schedule normally), decline / no-response after follow-up #2
  (Interceptly marks inactive, this plugin does nothing).
...
3. Inbound replies flow into `rockstarr-reply`'s daily loop once
...
- Do not edit `icp-qualifications.md` from this skill. That file
  is owned by `rockstarr-reply`. Per-campaign tightening lives in
  the campaign-specific ICP file only.
