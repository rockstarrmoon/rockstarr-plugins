...
description: "This skill should be used in the daily outreach loop after process-inbox completes for the active managed account, or when the user says \"process my tasks\", \"run the Interceptly My Tasks pass\", or \"handle overdue and due-today tasks\". Opens Interceptly then My Tasks, sorts by Due date oldest-first, runs qualify-lead per task, hands off to rockstarr-reply with the same handoff bundle. Same two modes as process-inbox. book-meeting tasks route directly to book-meeting in both modes (close was authorized on a prior turn). Walks every page — does not stop after page 1."
---
